...
def complete_belief_network(n_nodes: int, edge_values="default") -> nx.Graph:
    """create a fully connected belief network and with all edges set to a user input (1) uniform float value or (2) that specified by a list.
...
def add_colors_to_edges(G, binary=True):
    ...
    if binary:
        ...
        colors_for_edges = {e:{"color":gradient_colors[bisect(boundaries, G.edges[e]['weight'])]} for e in G.edges()}
...
def triad_energy(G: nx.Graph, triad, weight_key="weight") -> float:
...
def permute_stable_networks(n_nodes):
    ...
    for p in permutations:
        attr = {e:{'weight':b} for e,b in zip(edge_list, p)}
...
def simulate(sim_no, n_nodes, N=100, p=.2):
    ...
    for t in tqdm(range(T+1)):
        ...
        if t % per_interaction_to_track == 0:
            track[t] = {}
            beliefs = np.array([[G.nodes[n]['belief_network'].edges[e]['weight'] for e in edge_list] for n in G.nodes()])
